...
const makeDocSnap = (id, data) => ({ id, exists: () => Boolean(data), data: () => cloneDemo(data || {}) });
const makeQuerySnap = (rows) => ({ docs: rows.map(([id, data]) => ({ id, data: () => cloneDemo(data) })) });

function getCollectionName(source) {
  if (source?.type === "collection") return source.name;
  if (source?.type === "query") return getCollectionName(source.source);
  return "";
}
function applyQueryConstraints(rows, constraints = []) {
  let next = rows;
  for (const constraint of constraints) {
    ...
    if (constraint?.type === "limit") next = next.slice(0, Number(constraint.count || next.length));
  }
  return next;
}
async function setDoc(ref, data, options = {}) {
  const store = demoDatabase[ref.collectionName] || (demoDatabase[ref.collectionName] = {});
  const previous = store[ref.id] || {};
  store[ref.id] = options?.merge ? { ...previous, ...cloneDemo(data) } : cloneDemo(data);
  return true;
}
async function addDoc(collectionRef, data) {
  const store = demoDatabase[collectionRef.name] || (demoDatabase[collectionRef.name] = {});
  const id = `demo_${demoIdSeq++}`;
  store[id] = { ...cloneDemo(data), id };
  return { id };
}
async function updateDoc(ref, patch) {
  const store = demoDatabase[ref.collectionName] || (demoDatabase[ref.collectionName] = {});
  store[ref.id] = { ...(store[ref.id] || {}), ...cloneDemo(patch) };
  return true;
}
...
async function getDoc(ref) {
  const data = demoDatabase[ref.collectionName]?.[ref.id];
  return makeDocSnap(ref.id, data);
}
async function getDocs(source) {
  const name = getCollectionName(source);
  const rows = Object.entries(demoDatabase[name] || {});
  const filtered = source?.type === "query" ? applyQueryConstraints(rows, source.constraints) : rows;
...
function seedDemoDatabase() {
  ...
  demoDatabase.employees = {
    [DEV_PROFILE.userId]: {
      id: DEV_PROFILE.userId,
      lineUserId: DEV_PROFILE.userId,
      name: "Demo 管理者",
  ...
  demoDatabase.shiftTemplates = {
    shift_a_morning: { id: "shift_a_morning", name: "A店早班", department: "A店", startTime: "09:00", endTime: "18:00", scheduledMinutes: 540, graceMinutes: DEFAULT_GRACE_MINUTES, isRest: false, isFlexible: false, createdAt: now, updatedAt: now },
    shift_b_late: { id: "shift_b_late", name: "B店晚班", department: "B店", startTime: "10:00", endTime: "19:00", scheduledMinutes: 540, graceMinutes: DEFAULT_GRACE_MINUTES, isRest: false, isFlexible: false, createdAt: now, updatedAt: now },
    shift_rest: { id: "shift_rest", name: "休息", department: "休息", startTime: "", endTime: "", scheduledMinutes: 0, graceMinutes: 0, isRest: true, isFlexible: false, createdAt: now, updatedAt: now },
  };
  demoDatabase.schedules = {
    sch_a_today: { id: "sch_a_today", employeeId: "emp_a", employeeName: "陳小安", department: "A店", date, month, shiftId: "shift_a_morning", shiftName: "A店早班", isRest: false, isFlexible: false, startTime: "09:00", endTime: "18:00", scheduledMinutes: 540, graceMinutes: DEFAULT_GRACE_MINUTES, note: "Demo 排班", status: "scheduled", createdAt: now, updatedAt: now },
    sch_b_today: { id: "sch_b_today", employeeId: "emp_b", employeeName: "林柏宇", department: "B店", date, month, shiftId: "shift_b_late", shiftName: "B店晚班", isRest: false, isFlexible: false, startTime: "10:00", endTime: "19:00", scheduledMinutes: 540, graceMinutes: DEFAULT_GRACE_MINUTES, note: "Demo 排班", status: "scheduled", createdAt: now, updatedAt: now },
  };
  demoDatabase.attendanceRecords = {
    rec_a_today: { id: "rec_a_today", employeeId: "emp_a", employeeName: "陳小安", department: "A店", date, month, scheduleId: "sch_a_today", scheduledStart: "09:00", scheduledEnd: "18:00", clockIn: "09:02", clockOut: "18:00", workMinutes: 538, workHours: 8.97, attendanceStatus: "normal", lateMinutes: 0, earlyLeaveMinutes: 0, source: "demo", locationVerified: true, locationName: "A店", status: "completed", createdAt: now, updatedAt: now },
    rec_b_today: { id: "rec_b_today", employeeId: "emp_b", employeeName: "林柏宇", department: "B店", date, month, scheduleId: "sch_b_today", scheduledStart: "10:00", scheduledEnd: "19:00", clockIn: "10:15", clockOut: "19:00", workMinutes: 525, workHours: 8.75, attendanceStatus: "late", lateMinutes: 15, earlyLeaveMinutes: 0, source: "demo", locationVerified: true, locationName: "B店", status: "completed", createdAt: now, updatedAt: now },
  };
  demoDatabase.correctionRequests = {
    corr_demo: { id: "corr_demo", employeeId: "emp_a", employeeName: "陳小安", department: "A店", type: "clockIn", date, time: "09:00", reason: "忘記打上班卡", status: "pending", createdAt: now, updatedAt: now },
  };
  demoDatabase.payrollAdjustments = {
    adj_demo: { id: "adj_demo", employeeId: "emp_a", employeeName: "陳小安", month, type: "addition", title: "Demo 獎金", amount: 500, note: "作品集展示資料", createdAt: now, updatedAt: now },
  };
}
...
async function getClockPosition(companyLocations) {
  if (DEMO_NO_FIREBASE) return buildDevPosition(companyLocations);
  try {
...
if (DEV_MODE) {
  ...
  setTodayRecords(Object.entries(demoDatabase.attendanceRecords).map(([id, data]) => ({ id, ...data })).filter((record) => record.employeeId === lineProfile.userId && record.date === todayString()));
  setTodaySchedules(Object.entries(demoDatabase.schedules).map(([id, data]) => ({ id, ...data })).filter((schedule) => schedule.employeeId === lineProfile.userId && schedule.date === todayString()));
  setLoading(false);
  return;
}
  ...
  return (
    <div className="min-h-screen bg-neutral-100 text-neutral-900">
      <header className="sticky top-0 z-10 border-b bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <div>
            ...
            <p className="text-xs text-neutral-500">作品集 Demo｜本地記憶體資料｜不連 Firebase / LIFF</p>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-5">
        ...
        {DEV_MODE && <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">目前是作品集 Demo 模式：所有資料只存在瀏覽器記憶體，不讀取、不寫入 Firebase。</div>}
